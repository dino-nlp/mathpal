...
def configure_opik() -> None:
    # logger.info(f"COMET_API_KEY: {settings.COMET_API_KEY}")
    # logger.info(f"COMET_PROJECT: {settings.COMET_PROJECT}")
    # logger.info(f"COMET_WORKSPACE: {settings.COMET_WORKSPACE}")
    if settings.COMET_API_KEY and settings.COMET_PROJECT:
        ...
        logger.info(f"COMET_DEFAULT_WORKSPACE: {default_workspace}")
        opik.configure(
