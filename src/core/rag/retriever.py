...
class VectorRetriever:
    ...
    def _search_single_query(self, generated_query: str, grade_id: str, k: int):
    ...
    def retrieve_top_k(self, k: int, to_expand_to_n_queries: int) -> list:
        ...
        grade_id = self._metadata_extractor.generate_response(self.query)
        if grade_id:
            logger.info(
                "Successfully extracted the grade_id from the query.",
                grade_id=grade_id,
            )
        else:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            search_tasks = [
                executor.submit(self._search_single_query, query, grade_id, k)
